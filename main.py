...
# Device setting (CPU by default if no CUDA)
...
def train(epoch):
    ...
    for batch_idx, (normal_inputs, anomaly_inputs) in enumerate(zip(normal_train_loader, anomaly_train_loader)):
        ...
        loss = criterion(outputs, batch_size, device=device)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        train_loss += loss.item()
    print(f'loss = {train_loss/len(normal_train_loader):.4f}')
    scheduler.step()
...
def test_abnormal(epoch):
    ...
    with torch.no_grad():
        for i, (data, data2) in enumerate(zip(anomaly_test_loader, normal_test_loader)):
            ...
            # Perhitungan Akurasi
            predictions = (score_list3 > 0.5).astype(int) # Menggunakan ambang batas 0.5
            correct_predictions += np.sum(predictions == gt_list3)
            total_predictions += len(gt_list3)
            
            # Perhitungan AUC
            fpr, tpr, thresholds = metrics.roc_curve(gt_list3, score_list3, pos_label=1)
            auc += metrics.auc(fpr, tpr)

        print(f'auc = {auc/140:.4f}')
